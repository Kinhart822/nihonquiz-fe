...
export const ClassOverviewCard: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border-2 border- dark:border-slate-700 shadow-sm flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col gap-4">
        {mockClasses.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-gray-500 dark:text-slate-400 font-medium">
            Không có dữ liệu
          </div>
        ) : (
          mockClasses.map((c) => (
            <div
              key={c.id}
              className="p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl flex items-center justify-between cursor-pointer hover:bg-teal-50 dark:hover:bg-teal-900/10 transition-colors border-2 border- hover:border-teal-200 dark:hover:border-teal-900/30"
            >
              <div className="flex items-center gap-3">
                <img
                  src={c.teacherAvatar}
