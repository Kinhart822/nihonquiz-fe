...
import { mockClasses } from '../../dashboard/mock/mockClasses';
    ...
    <>
      <div className="w-full max-w-6xl mx-auto px-4 py-8 md:px-8 md:py-12">
        ...
        {mockClasses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-4 bg-white dark:bg-slate-900 border-2 border- dark:border-slate-700 rounded-3xl shadow-sm">
            <div className="w-16 h-16 bg-gray-50 dark:bg-slate-800/50 rounded-2xl flex items-center justify-center mb-4">
              <GraduationCap className="w-8 h-8 text-gray-400 dark:text-slate-500" />
            </div>
            <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-1">
              Không có lớp học nào
            </h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 text-center max-w-sm">
              Bạn hiện chưa tham gia lớp học nào. Hãy đăng ký một khóa học để
              bắt đầu nhé!
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {mockClasses.map((cls) => (
              <div
                key={cls.id}
                className="bg-white dark:bg-slate-900 rounded-3xl p-6 border-2 border- dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden"
              >
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-slate-700 relative z-10">
