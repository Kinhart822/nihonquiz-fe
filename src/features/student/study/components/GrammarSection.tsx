...
import { mockGrammarTopics } from '../mock/mockGrammar';
import { TopicCard } from './TopicCard';
import { FilterOption } from '../../../../shared/components/FilterOption';
import { FilterCheckbox } from '../../../../shared/components/FilterCheckbox';
...
export const GrammarSection: React.FC = () => {
  ...
  const filteredTopics = useMemo(() => {
    return mockGrammarTopics.filter((topic) => {
      const matchSearch =
        ...
        {filteredTopics.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredTopics.map((topic) => (
              <TopicCard key={topic.id} topic={topic} categoryType="Grammar" />
            ))}
          </div>
        ) : (
