...
export const PracticeTestPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F0FDFA] dark:bg-slate-950 text-[#134E4A] flex flex-col font-sans">
      {/* Header */}
      <header className="flex-shrink-0 h-16 px-4 md:px-8 border-b border-teal-100 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-50">
        <button
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-8 flex flex-col items-center">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Question Card */}
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-12 shadow-sm border border-teal-100 dark:border-slate-800 text-center relative">
              ...
              {currentQuestion.options.map((opt) => {
                ...
                if (status === 'idle' && isSelected) {
                  ...
                  if (isCorrect) {
                    ...
                    btnClass =
                      'border-2 bg-gray-50 dark:bg-slate-900/50 text-gray-400 dark:text-slate-500 border-gray-200 dark:border-slate-800 opacity-50';
                  }
                }
